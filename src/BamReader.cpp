...
char fourbits2base(uint8_t val)
{
    ...
    case 15:
        return 'N';
    default:
        cerr << "ERROR: Wrong base with value " << (int)val << std::endl;
        return 'N';
    }
}

char fourbits2base(uint8_t val, string &n)
{
    switch (val)
    {
    case 1:
    ...
    case 15:
        return 'N';
    default:
        cerr << "ERROR: Wrong base with value " << (int)val << " " << n << std::endl;
        return 'N';
    }
}
...
string getSeqFu8(uint8_t *data, int len, string &n)
{
    string s(len, '\0');
    for (int i = 0; i < len; i++)
    {
        char base;
        if (i % 2 == 1)
            base = fourbits2base(data[i / 2] & 0xF, n);
        else
            base = fourbits2base((data[i / 2] >> 4) & 0xF, n);
        s[i] = base;
    }
    delete[] data;
    return s;
...
{
    auto s = sam_hdr_tid2name(bam_header, aln->core.tid);
    if (s)
        return s;
    else
        return "NA";
